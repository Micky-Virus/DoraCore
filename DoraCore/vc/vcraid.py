...
from DoraCore.vc.queues import QUEUE, add_to_queue, get_queue, clear_queue
...
from .. import (call_py1, call_py2, call_py3, call_py4,
                    ...
                    HNDLR, SUDO_USERS, Dora1)

logging.basicConfig(level=logging.INFO)

aud_list = [
    "./DoraCore/Audio/AUD1.mp3",
    "./DoraCore/Audio/AUD2.mp3",
    "./DoraCore/Audio/AUD3.mp3",
    "./DoraCore/Audio/AUD4.mp3",
    "./DoraCore/Audio/AUD5.mp3",
    "./DoraCore/Audio/AUD6.mp3",
    "./DoraCore/Audio/AUD7.mp3",
    "./DoraCore/Audio/AUD8.mp3",
    "./DoraCore/Audio/AUD9.mp3",
    "./DoraCore/Audio/AUD10.mp3",
]
...
async def vcraid(_, e: Message):
    ...
    if gid == uid:
        inp = e.text[8:]
        chat_ = await Dora1.get_chat(inp)
        chat_id = chat_.id
    else:
         chat_id = gid
    aud = choice(aud_list)
    if inp:
        TheDoraXD = await e.reply_text("**Starting VC raid**")
        link = f"https://itshellboy.tk/{aud[1:]}"
        dl = aud
        songname = aud[18:]
        if chat_id in QUEUE:
            pos = add_to_queue(chat_id, songname, dl, link, "Audio", 0)
            await TheDoraXD.delete()
